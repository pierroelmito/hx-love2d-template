...
Game.prototype.genImage = function(self,w,h,bilinear,f) 
  ...
  local r = love.graphics.newImage(id);
...
Game.prototype.load = function(self,args) 
  love.window.setTitle("yolo");
  love.mouse.setVisible(false);
  self.img1 = self:genImage(16, 16, false, function(x,y) 
    do return _hx_o({__fields__={r=true,g=true,b=true,a=true},r=_hx_bit.band((_hx_bit.bxor(x,y)),1),g=1,b=1,a=1}) end;
  end);
  self.img2 = self:genImage(16, 16, true, function(x1,y1) 
    local dx = (x1 - 7.5) / 7.5;
    local dy = (y1 - 7.5) / 7.5;
    local v = 1 - Math.min(1, _G.math.sqrt((dx * dx) + (dy * dy)));
    do return _hx_o({__fields__={r=true,g=true,b=true,a=true},r=1,g=1,b=1,a=v}) end;
  end);
  self.img3 = love.graphics.newImage("crosshair.png");
end
Game.prototype.draw = function(self) 
  local _hx_1_mode_width, _hx_1_mode_height, _hx_1_mode_flags = love.window.getMode();
  local _hx_2_sz_width, _hx_2_sz_height = self.img1:getDimensions();
  local cx = 0.5 * _hx_1_mode_width;
  local cy = 0.5 * _hx_1_mode_height;
  local s1 = 80.0;
  local s2 = 20.0;
  local r = 150.0;
  love.graphics.setColor(1, 1, 1, 1);
  love.graphics.draw(self.img1, cx, cy, -0.01 * self.frame, s1, s1, 0.5 * _hx_2_sz_width, 0.5 * _hx_2_sz_height);
  love.graphics.setColor(1, 0, 0, 1);
  love.graphics.draw(self.img2, cx + (r * _G.math.cos(0.07 * self.frame)), cy + (r * _G.math.sin(0.08 * self.frame)), 0, s2, s2, 0.5 * _hx_2_sz_width, 0.5 * _hx_2_sz_height);
  if (self.vis) then 
    love.graphics.setColor(0.0, 1.0, 0.0, 1.0);
    love.graphics.draw(self.img3, self.x - 8, self.y - 8);
  end;
end
Game.prototype.update = function(self,dt) 
  local tmp = self;
  tmp.frame = tmp.frame + (dt * 60.0);
end
Game.prototype.keypressed = function(self,k,c,b) 
  if (c == "escape") then 
    love.event.quit();
  end;
end
Game.prototype.touchmoved = function(self,id,x,y,dx,dy,pressure) 
  self.x = x;
  self.y = y;
end
Game.prototype.touchpressed = function(self,id,x,y,dx,dy,pressure) 
  self.vis = true;
end
Game.prototype.touchreleased = function(self,id,x,y,dx,dy,pressure) 
  self.vis = false;
end
...
Main.main = function() 
  local g = Game.new();
  love.load = _hx_bind(g,g.load);
  love.draw = _hx_bind(g,g.draw);
  love.update = _hx_bind(g,g.update);
  love.keypressed = _hx_bind(g,g.keypressed);
  love.touchmoved = _hx_bind(g,g.touchmoved);
  love.touchpressed = _hx_bind(g,g.touchpressed);
  love.touchreleased = _hx_bind(g,g.touchreleased);
end
